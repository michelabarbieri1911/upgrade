...
\label{future}
In the rank 2 example we presented in Section \ref{rank2example}, the GIT quotient $X_1$ is a toric Calabi-Yau threefold of Picard rank 2. Given any toric Calabi-Yau threefold $X$ of Picard rank 2, we can consider the brane monodromy conjecture by constructing a toric Calabi-Yau GIT problem $(\CC^*)^2$ acting on $\CC^5$ for which $X$ is a GIT quotient. Note that we know the rank of the GIT problem 2 because of the rank of the Picard group, and we know the dimension of the problem is 5 because the quotient $X$ is three-dimensional. Our problem is therefore given by exact sequence
$$
N \xrightarrow{A} \Z^5 \xrightarrow{Q} L^v
$$
with $Q$ the weight matrix and $\text{ker} (Q) = A$, where $N\cong \Z^3$ and $ L^v \cong \Z^2$. Recall from Section \ref{fips} the definition of the primary polygon $P \in \Z^3 \otimes_{\Z} \R$, which is the convex hull of the rays $a_i = A^T(e_i) \in \Z^{3}$. 
Because of our Calabi-Yau assumption (the weights sum up to 0), we can choose generators for $N$ so that the first column of $A$ is a list of 1s, and we see $P$ living in the rank 2 lattice in $\Z^3$ where the first integer coordinate is 1. Imposing the first column of $A$ to be 1 does not uniquely determine generators for $N$. More precisely, these matrix representations of $A$ are unique up to right multiplication by 
matrices in $GL_3(\Z)$ given in block form as
\[ B =
...
\end{pNiceArray}
\]
where $t_1,t_2 \in \Z$ and $M \in GL_{2} (\Z)$.
This the primary polygon $P\subset \Z^2$ is only unique up to an action of $\Z^2 \rtimes GL_2(\Z)$\footnote{Multiplication in $\Z^2 \rtimes GL_2(\Z)$ is given by
    $$
    \left(\begin{pmatrix}
       t_1 \\
       t_2
...
$$
 \left(\begin{pmatrix}
    t_1 \\
    t_2
 \end{pmatrix}, M \right) \cdot \begin{pmatrix}
    x_1 \\
    x_2
 ...
 \end{pmatrix}.
$$
Because we know $A^T$ is surjective, we know that the primary polytope $P$has exactly 5 integer points in it. This means that toric Calabi-Yau threefold of Picard rank 2 come from convex lattice polygons in $\Z^2$ with exactly 5 integer points, up to the $\Z^2 \rtimes GL_2(\Z)$ action. We have proved that there are only six such classes of  polygons, see Figure \ref{CY3rank2}.
\begin{figure}[!h]
    \centering
